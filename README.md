...
Markwell is an open-source Markdown viewer and editor for macOS and Linux.
...
+-- Cargo.toml
+-- crates/
|   +-- markdown-core/             parser, semantic IR, diagnostics
|   +-- markdown-render-terminal/  terminal renderer and Mermaid text output
|   +-- markdown-render-html/      HTML renderer for desktop/system previews
|   +-- markdown-app/              active `md` binary: CLI + Tauri desktop app
|   +-- markdown-cli/              legacy standalone CLI crate, not in workspace
|   `-- markdown-ffi/              C ABI bridge for system integration adapters
...
cargo run -p markdown-app -- tests/fixtures/sample.md --no-tui --plain
cargo run -p markdown-app -- tests/fixtures/sample.md
cargo run -p markdown-app -- --app
```
...
```bash
cargo build --release -p markdown-app
```
...
markdown-core
   |
   +--> markdown-render-terminal --> md CLI / ratatui viewer
   |
   +--> markdown-render-html -----> Tauri desktop preview
   |
   `--> markdown-ffi -------------> future Quick Look/system adapters
```
